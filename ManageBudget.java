...
public class ManageBudget extends javax.swing.JFrame {
        ...
        if(checkInputs()){
            ...
            if (checkCatgeoryForBudget()) {
                ...
                try {
                    ...
                    ps.setFloat(4, (Float.parseFloat(jTextField_Spending.getText()) + getLastSpending()));
                    ps.setString(5, date);
    ...
    public void Spending_Over_80() {
        ...
        try {
            ...
            if(rs.next()) {
                ...
                if (getLastSpending() >= (80 * budget) / 100.0) 
    ...
    //get the last Spending Amount
    public float getLastSpending(){
        String query = "select spendingAmount "
                + " from spending "
                + " where budgetID = ? " 
                + " and ModificationDate = (select max(ModificationDate) "
                + " from spending "
                + " where budgetID = ?)";
        PreparedStatement ps;
        ResultSet rs;
        float lastSpending = -1;
        
        try {
            ps = MyConnection.getConnection().prepareStatement(query);
            ps.setInt(1, getBudgetID());
            ps.setInt(2, getBudgetID());
            
            rs = ps.executeQuery();
            if (rs.next()) {
                lastSpending = rs.getFloat("SpendingAmount");
            }
        } catch (SQLException ex) {
            Logger.getLogger(ManageBudget.class.getName()).log(Level.SEVERE, null, ex);
        }
